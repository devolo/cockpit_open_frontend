...
class DrawNetworkOverview extends CustomPainter {
  ...
  void drawDeviceIconContent(Canvas canvas, int deviceIndex) {
    ...
    if (showingSpeeds && deviceIndex != pivotDeviceIndex) {
      ...
      if (_deviceList.elementAt(pivotDeviceIndex).speeds[
      macToCanonical(_deviceList.elementAt(deviceIndex).mac)] !=
          null) {
        rx = _deviceList
            .elementAt(pivotDeviceIndex)
            .speeds[macToCanonical(_deviceList.elementAt(deviceIndex).mac)]
            .rx
            .round();
      ...
      _speedTextPainter.paint(
          canvas,
          Offset(absoluteCenterOffset.dx - (_speedTextPainter.width / 2),
  ...
  void drawPCIcon(Canvas canvas, Size size) {
    ...
    Rect pcRect = new Rect.fromPoints(
        Offset(absolutePCOffset.dx - 20, absolutePCOffset.dy - 15),
        Offset(absolutePCOffset.dx + 20, absolutePCOffset.dy + 15));
    canvas.drawRect(pcRect, _pcPaint);

    pcRect = new Rect.fromPoints(
        Offset(absolutePCOffset.dx - 30, absolutePCOffset.dy + 16),
        Offset(absolutePCOffset.dx + 30, absolutePCOffset.dy + 30));
    canvas.drawRect(pcRect, _pcPaint);

    if (_deviceList.length > 0)
      canvas.drawLine(Offset(absolutePCOffset.dx, absolutePCOffset.dy + 60),
          absolutePCDeviceOffset, _linePaint);
    ...
    _textPainter.paint(
        canvas,
        Offset(absolutePCOffset.dx - (_textPainter.width / 2),
            absolutePCOffset.dy + (20 + _textPainter.height)));
  }
  ...
  void drawRouterIcon(Canvas canvas, Size size) {
    ...
    Rect routerRect = new Rect.fromPoints(
        Offset(absoluteRouterOffset.dx - 30, absoluteRouterOffset.dy - 15),
        Offset(absoluteRouterOffset.dx + 30, absoluteRouterOffset.dy + 15));
    canvas.drawRect(routerRect, _routerPaint);

    if (_deviceList.length > 0)
      canvas.drawLine(
          Offset(absoluteRouterOffset.dx, absoluteRouterOffset.dy + 50),
          absoluteRouterDeviceOffset,
          _linePaint);
    ...
    _textPainter.paint(
        canvas,
        Offset(absoluteRouterOffset.dx - (_textPainter.width / 2),
            absoluteRouterOffset.dy + (10 + _textPainter.height)));
  }
  ...
  void drawAllDeviceIcons(Canvas canvas, Size size) {
    ...
    for (int numDev = 0; numDev < _deviceList.length; numDev++) {
      ...
      if (numDev != pivotDeviceIndex)
        drawDeviceName(
            canvas,
            size,
            _deviceList.elementAt(numDev).type,
            _deviceList.elementAt(numDev).name,
            _deviceIconOffsetList.elementAt(numDev));
    }
    ...
    if (_deviceList.length > 0) {
      ...
      drawDeviceName(
          canvas,
          size,
          _deviceList.elementAt(pivotDeviceIndex).type,
          _deviceList.elementAt(pivotDeviceIndex).name,
          _deviceIconOffsetList.elementAt(pivotDeviceIndex));
    } else {
